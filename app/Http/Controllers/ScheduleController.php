            	...
            	if($time == $now){
			    	...
			    	foreach ($alerts as $alert) {
		    			...
		    			if ($alert_one == $today) {
				        	...
				        	'. This is a reminder that your Service (Service ID: '. $alert->loan->loan_identity . ') payment of Tsh '. number_format($alert->schedule->amount, 2) . '/- is due on '. Carbon::parse($alert->schedule->date)->toFormattedDateString() . ' at our ' . $alert->loan->office->name . ' office. For more information please call +' . $alert->tenant->sales->phone_number . '.' . PHP_EOL . 'Thank You - '. $alert->tenant->name;

				        	$swaMessage = 
				        	'Habari Ndugu '. 
				        	$alert->loan->client->first_name . ' ' . $alert->loan->client->last_name . 
				        	'. Ujumbe huu nikukukumbusha malipo ya Huduma yako (Huduma No: '. $alert->loan->loan_identity . ') kiasi cha Tsh '. number_format($alert->schedule->amount, 2) . '/- kinatakiwa tarehe '. Carbon::parse($alert->schedule->date)->toFormattedDateString() . ' kwenye ofisi yetu ya ' . $alert->loan->office->name . '. Kwa maelezo zaidi piga namba +' . $alert->tenant->sales->phone_number . '.' . PHP_EOL . 'Asante - '. $alert->tenant->name;
				        	...
				        	'Habari Ndugu '. 
				        	$alert->loan->client->first_name . ' ' . $alert->loan->client->last_name . 
				        	'. Ujumbe huu nikukukumbusha malipo ya Huduma yako (Service ID: '. $alert->loan->loan_identity . ') kiasi cha Tsh '. number_format($alert->schedule->amount, 2) . '/- kinatakiwa tarehe '. Carbon::parse($alert->schedule->date)->toFormattedDateString() . ' kwenye ofisi yetu ya ' . $alert->loan->office->name . '. Kwa maelezo zaidi piga namba +' . $alert->tenant->sales->phone_number . '.' . PHP_EOL . 'Asante - '. $alert->tenant->name;
				        	...
				        	'Hello '. 
				        	$alert->loan->client->first_name . ' ' . $alert->loan->client->last_name . 
				        	'. This is a reminder that your Service (Service ID: '. $alert->loan->loan_identity . ') payment of Tsh '. number_format($alert->schedule->amount, 2) . '/- is due today on '. Carbon::parse($alert->schedule->date)->toFormattedDateString() . ' at our ' . $alert->loan->office->name . ' office. For more information please call +' . $alert->tenant->sales->phone_number . '.' . PHP_EOL . 'Thank You - '. $alert->tenant->name;

				        	$swaMessage = 
				        	'Habari Ndugu '. 
				        	$alert->loan->client->first_name . ' ' . $alert->loan->client->last_name . 
				        	'. Ujumbe huu nikukukumbusha malipo ya Huduma yako (Huduma ID: '. $alert->loan->loan_identity . ') kiasi cha Tsh '. number_format($alert->schedule->amount, 2) . '/- yanatakiwa leo tarehe '. Carbon::parse($alert->schedule->date)->toFormattedDateString() . ' kwenye ofisi yetu ya ' . $alert->loan->office->name . '. Kwa maelezo zaidi piga namba +' . $alert->tenant->sales->phone_number . '.' . PHP_EOL . 'Asante - '. $alert->tenant->name;
