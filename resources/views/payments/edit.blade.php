...
<!-- Page Header -->
@section('header')Edit Service @stop

<!-- Page Description -->
@section('desc')Update Service Details @stop
	...
	<div class="col-md-12"> 
        ...
        <div class="wizard-container">
            <div class="card wizard-card" data-color="blue" id="wizard">
                <form action="{{ route('payments.update', array($payment->id)) }}" method="POST" accept-charset="UTF-8">
                    <input type="hidden" name="_method" value="PUT">
                    <input name="_token" value="{{ csrf_token() }}" type="hidden">
                    
                    <div class="wizard-header">
                        <h3 class="wizard-title">
                            Edit Service Payment Details
                        </h3>
                    ...
                    <div class="tab-content">
                        <div class="tab-pane" id="details">
                            <div class="row">
                                <div class="col-sm-12">
                                    <h4 class="info-text">Service ID:{{ $payment->loan->id }} | {{ $payment->loan->client->first_name }} {{ $payment->loan->client->middle_name }} {{ $payment->loan->client->last_name }}</h4>
                                </div>
                                <div class="col-sm-10">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-group label-floating">
                                            ...
                                            <input name="date" value="{{ $payment->date }}" type="date" class="form-control">
                                        </div>
                                    </div>
                                            ...
                                            <label class="control-label">Payment Method</label>
                                                <select class="form-control" name="method">      
                                                    ...
                                                    @foreach($methods as $method)
                                                        <option value="{{ $method->id }}" {{ $payment->method->id == $method->id ? 'selected' : '' }}>{{ $method->name }}</option>
                                                    @endforeach
                                                </select>
                                        </div>
                                    </div>
