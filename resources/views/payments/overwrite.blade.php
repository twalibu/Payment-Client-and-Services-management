...
<!-- Page Title -->
@section('title')Service Overwrite @stop
...
<!-- Page Header -->
@section('header')Service Overwrite @stop

<!-- Page Description -->
@section('desc')Overwrite Service Payment @stop

<!-- Active Link -->
@section('active')Service Overwrite @stop
	...
	<div class="col-md-12"> 
        ...
        <div class="wizard-container">
            <div class="card wizard-card" data-color="blue" id="wizard">
                <form action="{{ route('payment.overwrite') }}" method="POST" accept-charset="UTF-8">
                    <input name="_token" value="{{ csrf_token() }}" type="hidden">
                    <input name="loan" value="{{ $loan->id }}" type="hidden">
                    
                    <div class="wizard-header">
                        <h3 class="wizard-title">
                            New Service Payment Overwrite
                        </h3>
                    ...
                    <div class="tab-content">
                        <div class="tab-pane" id="details">
                            <div class="row">
                                <div class="col-sm-12">
                                    <h4 class="info-text">Service ID: {{ $loan->loan_identity }} | {{ $loan->client->first_name }} {{ $loan->client->middle_name }} {{ $loan->client->last_name }}</h4>
                                </div>
                                <div class="col-sm-10">
                                                ...
                                                <select class="form-control" name="schedule">      
                                                    ...
                                                    @foreach($loan->schedules as $schedule)
                                                        @if($schedule->status != "Completed")
                                                            <option value="{{ $schedule->id }}">Month {{ $schedule->month }} | Balance Amount - Tsh {{ number_format($schedule->balance, 2) }}</option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                        </div>
